...
public class MovieCrewAdapter extends RecyclerView.Adapter<MovieCrewAdapter.MoviesItemViewHolder> {
    private Context context;
    private List<MovieCastCrew.CrewBean> crewArrayList;
    private LayoutInflater mInflater;

    public MovieCrewAdapter(Context mContext) {
    ...
    public void onBindViewHolder(@NonNull MoviesItemViewHolder viewHolder, final int i) {
        MovieCastCrew.CrewBean crewBean = crewArrayList.get(i);
        viewHolder.title.setText(crewBean.getName());
        viewHolder.subtitle.setText(crewBean.getDepartment());
        Picasso.with(viewHolder.itemImage.getContext()).load(AppUtil.movieImagePathBuilder(crewBean.getProfile_path())).placeholder(R.drawable.cast_crew_placeholder).fit().centerCrop().into(viewHolder.itemImage);
    }
    ...
    public void addDataIntoList(List<MovieCastCrew.CrewBean> crewBeanList) {
        crewArrayList.clear();
        crewArrayList.addAll(crewBeanList);
